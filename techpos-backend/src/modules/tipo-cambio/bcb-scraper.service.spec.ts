...
describe('BcbScraperService', () => {
  let tipoCambioService: jest.Mocked<Pick<TipoCambioService, 'sincronizarDiario' | 'upsertDiario' | 'findUltimo'>>;
  let scraper: BcbScraperService;
  ...
  beforeEach(() => {
    ...
    scraper = new BcbScraperService(tipoCambioService as unknown as TipoCambioService);
  });
  ...
  describe('parsearTcoVigente', () => {
    ...
    it('lanza error si no existe el widget de tipo de cambio', () => {
      expect(() => scraper.parsearTcoVigente('<div>vacío</div>')).toThrow(
        'No se encontró el widget de tipo de cambio oficial',
      );
    });
  ...
  describe('obtenerVigenteConFallback (RF-1.2)', () => {
    ...
    it('lanza error si el scraping falla y no hay histórico en BD', async () => {
      ...
      await expect(scraper.obtenerVigenteConFallback()).rejects.toThrow(
        'Scraping falló y no existe ninguna cotización histórica en BD',
      );
    });
  });
